...
values <- mapply('/', count_table %>% dplyr::summarise(across(starts_with("GSM"), ~./Length*1000*1000000)), mapped_reads[,-1])
...
head(rpkm_table)
write.table(rpkm_table, file="RPF_rpkm_table.tsv", sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
