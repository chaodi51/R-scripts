...
write.table(data.frame("Name"=rownames(res),res), "riborex_res_deseq2.tsv", quote=FALSE, sep='\t',row.names=F, col.names=T)
...
```{r include=TRUE, echo=FALSE, message=FALSE, warning = FALSE}
gprofilter_func <- function(genelist, pval_cutoff){
    ...
    pp <- publish_gostplot(p, highlight_terms = gostres$result[gostres$result$p_value<pval_cutoff,]$term_id, 
                            height = 20, width = 10, filename = NULL)
...
* For all DEGs
```{r include=TRUE, echo=FALSE, message=FALSE, warning = FALSE, fig.height=24, fig.width=12}
gprofilter_func(genelist=DEGs, pval_cutoff=1e-2)
cat("Enrichment cutoff p<1e-2\n")
```

* For translation increase but transcription not increase (by p-value)
```{r include=TRUE, echo=FALSE, message=FALSE, warning = FALSE, fig.height=24, fig.width=12}
cat(paste0("ribo-Up genes: ", length(up_gene$padj),"\n"))

res_RNAseq <- read.table("../GSE120762_rnaseq/LPS_vs_WT.diffexp.tsv", header=T)
RNAUp <- res_RNAseq %>% dplyr::filter(padj<0.05 & log2FoldChange>=1)
RNADown <- res_RNAseq %>% dplyr::filter(padj<0.05 & log2FoldChange<=-1)

riboUp_RNAnotUp <- up_gene[!rownames(up_gene) %in%  RNAUp$Gene,]
write.table(data.frame("Name"=rownames(riboUp_RNAnotUp), riboUp_RNAnotUp), "riborex_res_riboUp_RNAnotUp.tsv", quote=FALSE, sep='\t',row.names=F, col.names=T)

cat(paste0("ribo-Up genes excluded RNA not Up: ", length(riboUp_RNAnotUp$padj),"\n"))
cat("Enrichment cutoff p<0.05\n")
gprofilter_func(genelist=riboUp_RNAnotUp, pval_cutoff=0.05)
```

* For translation decrease but transcription not decrease (by p-value)
```{r include=TRUE, echo=FALSE, message=FALSE, warning = FALSE, fig.height=24, fig.width=12}
cat(paste0("ribo-Down genes: ", length(down_gene$padj),"\n"))
riboDown_RNAnotDown <- down_gene[!rownames(down_gene) %in% RNADown$Gene,]

write.table(data.frame("Name"=rownames(riboDown_RNAnotDown), riboDown_RNAnotDown), "riborex_res_riboDown_RNAnotDown.tsv", quote=FALSE, sep='\t',row.names=F, col.names=T)
cat(paste0("ribo-Down genes excluded RNA not Down: ", length(riboDown_RNAnotDown$padj),"\n"))
cat("Enrichment cutoff p<0.05\n")
gprofilter_func(genelist=riboDown_RNAnotDown, pval_cutoff=0.05)
```
...
gseafunc <- function(gmtfile, pval_cutoff, gene_sets){
    ...
    fgseaRes_filtered <-fgseaRes[padj < pval_cutoff]
    ...
    # output the table
    fwrite(fgseaRes_filtered, file=paste0("enriched_genesets_", gene_sets,".tsv"), sep="\t", sep2=c("",",",""))
    
 
    cat("Number of pathways enriched: ")
    cat(dim(fgseaRes_filtered)[1])
    p <- ggplot(fgseaRes_filtered, aes(reorder(pathway, NES), NES, color=padj)) +
         ...
         coord_flip() +
         labs(x="Pathway", y="Normalized Enrichment Score", title= gene_sets, color="adjust.p", size="Count") + 
         theme_minimal()
    print(p)
}
```
...
gseafunc(gmtfile=gmtfile, pval_cutoff=0.25, "GSEA-Ontology") ## no enrichment for padj<0.8

cat("All gene sets: \n")
gmtfile = fgsea::gmtPathways("./geneSets/msigdb.v7.2.symbols.gmt")
gseafunc(gmtfile=gmtfile, pval_cutoff=0.25, "GSEA-All gene sets") ## no enrichment for padj<0.4

cat("C2.Curated gene sets-KEGG: \n")
