...
pcaData <- plotPCA(vsd_STAT1GOF, intgroup=c("disease","STAT1GOF_group","cell_type"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot(pcaData, aes(PC1, PC2, shape=STAT1GOF_group, color=disease)) +
  geom_point(size =3, aes(fill=disease, alpha=cell_type))+ 
  geom_point(size =3) + 
  scale_shape_manual(values=c(22,23,24)) + 
  scale_alpha_manual(values=c('Tnn'=0.05, 'Tn'=1)) + 
...
pcaData <- plotPCA(vsd_STAT1LOF, intgroup=c("disease","STAT1LOF_group","cell_type"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot(pcaData, aes(PC1, PC2, shape=STAT1LOF_group, color=disease)) +
  geom_point(size =3, aes(fill=disease, alpha=cell_type))+ 
  geom_point(size =3) + 
...
df <- as.data.frame(colData(dds)[,c("disease","STAT1GOF_group","cell_type")]) %>% filter(STAT1GOF_group!="NA")
pheatmap(
  pdata,
  cluster_rows = TRUE,
  show_rownames = TRUE,
...
df <- as.data.frame(colData(dds)[,c("disease","STAT1LOF_group","cell_type")]) %>% filter(STAT1LOF_group!="NA")
pheatmap(
  pdata,
  cluster_rows = TRUE,
  show_rownames = TRUE,
...
dds_STAT3GOF$group <- factor(paste0(dds_STAT3GOF$STAT3GOF_group, dds_STAT3GOF$disease, dds_STAT3GOF$cell_type))
design(dds_STAT3GOF) <- ~ group
dds_STAT3GOF <- DESeq(dds_STAT3GOF)
resultsNames(dds_STAT3GOF)

res_STAT3GOF_Young_Tnn <- results(dds_STAT3GOF, contrast=c("group", "YoungSTAT3GOFTnn","YoungHCTnn"))
# STAT3GOF_Young_Tn only has one sample, no statistics can be applied
res_STAT3GOF_Adults_Tnn <- results(dds_STAT3GOF, contrast=c("group", "AdultsSTAT3GOFTnn","AdultsHCTnn"))
res_STAT3GOF_Adults_Tn <- results(dds_STAT3GOF, contrast=c("group", "AdultsSTAT3GOFTn","AdultsHCTn"))

table(res_STAT3GOF_Young_Tnn$padj<0.1)
data.frame(res_STAT3GOF_Young_Tnn[!is.na(res_STAT3GOF_Young_Tnn$padj) & res_STAT3GOF_Young_Tnn$padj<0.1,])
res_STAT3GOF_Young_Tnn[rownames(res_STAT3GOF_Young_Tnn)=="STAT3",]

table(res_STAT3GOF_Adults_Tnn$padj<0.1)
data.frame(res_STAT3GOF_Adults_Tnn[!is.na(res_STAT3GOF_Adults_Tnn$padj) & res_STAT3GOF_Adults_Tnn$padj<0.1,])
res_STAT3GOF_Adults_Tnn[rownames(res_STAT3GOF_Adults_Tnn)=="STAT3",]

table(res_STAT3GOF_Adults_Tn$padj<0.1)
data.frame(res_STAT3GOF_Adults_Tn[!is.na(res_STAT3GOF_Adults_Tn$padj) & res_STAT3GOF_Adults_Tn$padj<0.1,])
res_STAT3GOF_Adults_Tn[rownames(res_STAT3GOF_Adults_Tn)=="STAT3",]
```
...
dds_STAT3LOF$group <- factor(paste0(dds_STAT3LOF$disease, dds_STAT3LOF$STAT3LOF_group, dds_STAT3LOF$cell_type))
design(dds_STAT3LOF) <- ~ group
dds_STAT3LOF <- DESeq(dds_STAT3LOF)
resultsNames(dds_STAT3LOF)

res_STAT3LOF_Young_Tnn <- results(dds_STAT3LOF, contrast=c("group", "STAT3LOFYoungTnn","HCYoungTnn"))
res_STAT3LOF_Young_Tn <- results(dds_STAT3LOF, contrast=c("group", "STAT3LOFYoungTn","HCYoungTn"))

res_STAT3LOF_Older_Tnn <- results(dds_STAT3LOF, contrast=c("group", "STAT3LOFOlderTnn","HCOlderTnn"))
res_STAT3LOF_Older_Tn <- results(dds_STAT3LOF, contrast=c("group", "STAT3LOFOlderTn","HCOlderTn"))

# STAT3LOF_Oldest only has one Tn and one Tnn sample, no statistics can be applied

table(res_STAT3LOF_Young_Tnn$padj<0.1)
res_STAT3LOF_Young_Tnn[!is.na(res_STAT3LOF_Young_Tnn$padj) & res_STAT3LOF_Young_Tnn$padj<0.1,]
res_STAT3LOF_Young_Tnn[rownames(res_STAT3LOF_Young_Tnn)=="STAT3",]

table(res_STAT3LOF_Young_Tn$padj<0.1)
res_STAT3LOF_Young_Tn[!is.na(res_STAT3LOF_Young_Tn$padj) & res_STAT3LOF_Young_Tn$padj<0.1,]
res_STAT3LOF_Young_Tn[rownames(res_STAT3LOF_Young_Tn)=="STAT3",]

table(res_STAT3LOF_Older_Tnn$padj<0.1)
res_STAT3LOF_Older_Tnn[!is.na(res_STAT3LOF_Older_Tnn$padj) & res_STAT3LOF_Older_Tnn$padj<0.1,]
res_STAT3LOF_Older_Tnn[rownames(res_STAT3LOF_Older_Tnn)=="STAT3",]

table(res_STAT3LOF_Older_Tn$padj<0.1)
res_STAT3LOF_Older_Tn[!is.na(res_STAT3LOF_Older_Tn$padj) & res_STAT3LOF_Older_Tn$padj<0.1,]
res_STAT3LOF_Older_Tn[rownames(res_STAT3LOF_Older_Tn)=="STAT3",]
```
...
dds_STAT1GOF$cell_type <- factor(dds_STAT1GOF$cell_type, levels = c("Tn","Tnn"))

dds_STAT1GOF$group <- factor(paste0(dds_STAT1GOF$disease, dds_STAT1GOF$STAT1GOF_group, dds_STAT1GOF$cell_type))
design(dds_STAT1GOF) <- ~ group
dds_STAT1GOF <- DESeq(dds_STAT1GOF)
resultsNames(dds_STAT1GOF)

# STAT1GOF_Young only has one Tn and one Tnn sample, no statistics can be applied

res_STAT1GOF_Older_Tnn <- results(dds_STAT1GOF, contrast=c("group", "STAT1GOFOlderTnn","HCOlderTnn"))
res_STAT1GOF_Older_Tn <- results(dds_STAT1GOF, contrast=c("group", "STAT1GOFOlderTn","HCOlderTn"))

res_STAT1GOF_Adults_Tnn <- results(dds_STAT1GOF, contrast=c("group", "STAT1GOFAdultsTnn","HCAdultsTnn"))
res_STAT1GOF_Adults_Tn <- results(dds_STAT1GOF, contrast=c("group", "STAT1GOFAdultsTn","HCAdultsTn"))


table(res_STAT1GOF_Older_Tnn$padj<0.1)
res_STAT1GOF_Older_Tnn[!is.na(res_STAT1GOF_Older_Tnn$padj) & res_STAT1GOF_Older_Tnn$padj<0.1,]
res_STAT1GOF_Older_Tnn[rownames(res_STAT1GOF_Older_Tnn)=="STAT1",]

table(res_STAT1GOF_Older_Tn$padj<0.1)
res_STAT1GOF_Older_Tn[!is.na(res_STAT1GOF_Older_Tn$padj) & res_STAT1GOF_Older_Tn$padj<0.1,]
res_STAT1GOF_Older_Tn[rownames(res_STAT1GOF_Older_Tn)=="STAT1",]

table(res_STAT1GOF_Adults_Tnn$padj<0.1)
res_STAT1GOF_Adults_Tnn[!is.na(res_STAT1GOF_Adults_Tnn$padj) & res_STAT1GOF_Adults_Tnn$padj<0.1,]
res_STAT1GOF_Adults_Tnn[rownames(res_STAT1GOF_Adults_Tnn)=="STAT1",]

table(res_STAT1GOF_Adults_Tn$padj<0.1)
res_STAT1GOF_Adults_Tn[!is.na(res_STAT1GOF_Adults_Tn$padj) & res_STAT1GOF_Adults_Tn$padj<0.1,]
res_STAT1GOF_Adults_Tn[rownames(res_STAT1GOF_Adults_Tn)=="STAT1",]
```
...
dds_STAT1LOF$cell_type <- factor(dds_STAT1LOF$cell_type, levels = c("Tn","Tnn"))

dds_STAT1LOF$group <- factor(paste0(dds_STAT1LOF$disease, dds_STAT1LOF$cell_type))
design(dds_STAT1LOF) <- ~ group
dds_STAT1LOF <- DESeq(dds_STAT1LOF)
resultsNames(dds_STAT1LOF)

# STAT1LOF only has one Tn and one Tnn sample, no statistics can be applied
...
```{r }
## Check the histogram of raw p-values
# # STAT3GOF Young
# hist(res_STAT3GOF_Young_Tnn$pvalue)
# # # STAT3GOF Adults
# hist(res_STAT3GOF_Adults_Tnn$pvalue)
# hist(res_STAT3GOF_Adults_Tn$pvalue)
# # # STAT3LOF Young
# hist(res_STAT3LOF_Young_Tnn$pvalue)
# hist(res_STAT3LOF_Young_Tn$pvalue)
# # # STAT3LOF Older
# hist(res_STAT3LOF_Older_Tnn$pvalue)
# hist(res_STAT3LOF_Older_Tn$pvalue)
# # # STAT1GOF Older
# hist(res_STAT1GOF_Older_Tnn$pvalue)
# hist(res_STAT1GOF_Older_Tn$pvalue)
# # # STAT1GOF Adults
# hist(res_STAT1GOF_Adults_Tnn$pvalue)
# hist(res_STAT1GOF_Adults_Tn$pvalue)
```

### Volcano plots {-}
```{r message=FALSE, warning = FALSE, fig.height=8, fig.width=7, fig.show="hold"}
for (each in c("STAT3GOF_Young_Tnn", "STAT3GOF_Adults_Tnn", "STAT3GOF_Adults_Tn", "STAT3LOF_Young_Tnn", "STAT3LOF_Young_Tn", "STAT3LOF_Older_Tnn", "STAT3LOF_Older_Tn", "STAT1GOF_Older_Tnn", "STAT1GOF_Older_Tn","STAT1GOF_Adults_Tnn", "STAT1GOF_Adults_Tn")){
...
gseafunc <- function(gmtfile, pval_cutoff, gene_sets){
    for (each in c("STAT3GOF_Young_Tnn", "STAT3GOF_Adults_Tnn", "STAT3GOF_Adults_Tn", "STAT3LOF_Young_Tnn",
                   "STAT3LOF_Young_Tn", "STAT3LOF_Older_Tnn", "STAT3LOF_Older_Tn", "STAT1GOF_Older_Tnn",
                   "STAT1GOF_Older_Tn","STAT1GOF_Adults_Tnn", "STAT1GOF_Adults_Tn")){
      # print comparison
      comp <- paste0(each, "_disease_vs_HC") 
      print(comp)
      res = get(paste0("res_",each))
