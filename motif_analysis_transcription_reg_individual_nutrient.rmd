---
title: "Motif analysis for differential transcripted genes by input total RNA-seq"
...
```{r, eval=FALSE}
inputRNA <- read.table("InputRNA.diffexp.tsv", header=TRUE)
...
get_seqs <- function(nutrient, up_down, utr_type, pval_cutoff, logfc, top_num){
    ...
    expr_change <- paste(nutrient, up_down, sep = "_")

    log2FC <- paste0("No", nutrient, "_log2FoldChange")
    padj <- paste0("No", nutrient, "_padj")
    
    if (up_down == "up") {
        expr_change <- inputRNA %>% 
            dplyr::select(contains(c("GeneSymbol", nutrient))) %>% 
            dplyr::filter(!!sym(log2FC) >= logfc, !!sym(padj) <= pval_cutoff) %>%
            dplyr::arrange(!!padj)
    } else {
        expr_change <- inputRNA %>% 
        dplyr::select(contains(c("GeneSymbol", nutrient))) %>% 
    ...
    cat("Number of input genes: ", dim(expr_change)[1], "\n")
    inputGenes <- head(expr_change, top_num)$GeneSymbol
...
for (tab_name in names(seqtab_list)) {
    ...
    set_dir = paste(proj_path,"/results/motif_analysis/transcription_reg_individual_nutrient/streme_out", tab_name, sep="/")
...
for (tab_name in names(streme_out_list)) {
    ...
    for (streme_motif in head(streme_out$motif,3)) {
        ...
        set_dir = paste0(proj_path, "/results/motif_analysis/transcription_reg_individual_nutrient/fimo_out", "/", tab_name, "/", streme_motif_name)
        mkdir(set_dir, r=TRUE)
...
### Run all conditions for transcriptional up and down genes, both 5'UTR and 3'UTR
```{r, eval=FALSE}

tomtom_out_list <- list()
for(tab_name in names(streme_out_list)) {
    set_dir = paste0(proj_path, "/results/motif_analysis/transcription_reg_individual_nutrient/tomtom_out", "/", tab_name)
    mkdir(set_dir, r=TRUE)
