---
title: "Motif analysis for Glucose (miRNA motifs)"
...
---
## Abstract {-}
1. Locate miRNA motifs from public databases.
...
### Run Fimo for all annotated miRNA motifs on UTRs
```{r, eval=FALSE}

# search RNA motifs in databases
RNAdb <- read_meme("/mnt/isilon/bailis_lab/projects/BailisW_polysome_profiling_312911604/resources/motif_databases/MIRBASE/22/Mus_musculus_mmu.meme")
...
for (motif in RNAdb) {
    ...
    if(grepl("-5p", motif_name)){
      print(motif_name)
      set_dir = paste0(proj_path, "/results/motif_analysis/translation_reg_glucose_miRNA_motif_selected/fimo_out", "/", motif_name)
...
### Prepare data to plot (selected 20 motifs) {.tabset}
* 5'UTR  
```{r, eval=FALSE}

all_motif_genes <- data.frame(matrix(ncol=7, nrow=0))
for (motif_name in head(names(fimo_out_list_utr5),2000)) { ## all motifs
    # print(motif_name)
...
all_motif_genes <- data.frame(matrix(ncol=7, nrow=0))
for (motif_name in head(names(fimo_out_list_utr3),2000)) { ## all motifs
    # print(motif_name)
...
#### plot translation changes for miRNA binding genes (5'UTR)

* Wilcoxon rank sum test for the miRNA binding genes vs. the rest of genes. 

```{r}
data5_1 <- readRDS("miRNA_motif_utr5.rds")
```
...
for (each in unique(data5$group)[grepl("mmu",unique(data5$group))]){
...
write.table(data5_1_output, "anota2seq_tab_for_genes_grouped_by_miRNA_motif_on_5UTR.tsv",sep="\t", col.names=TRUE,row.names = FALSE,quote=FALSE)

wilcox_stats <- data5_1 %>% group_by(group,wilcox_greater.p, wilcox_less.p) %>% count() %>% as.data.frame()
colnames(wilcox_stats)[4] <- "n_genes"
write.table(wilcox_stats, "translation_change_5UTR_miRNA_motif_genes_vs_allgene_wilcox_p.tsv", sep="\t", col.names=TRUE,row.names = FALSE,quote=FALSE)

saveRDS(data5_1, file="miRNA_motif_utr5.rds")
```
...
data5_1 <- rbind(data5_1, all_genes)
p <- ggplot(data5_1, aes(x=group, y=NoGlucose_translation.apvEff)) +
  ...
  xlab("Motif groups") +
  ggtitle("miRNA binding on 5'UTR") +
  ylim(-2,2) +

# Add p-values on top
 geom_text(
    data = data5_1[,c(1,7,12)] %>% group_by(group, wilcox.p) %>% summarise(y = quantile(NoGlucose_translation.apvEff, 0.75)),
...
#### plot translation changes for miRNA binding genes (3'UTR)

* Wilcoxon rank sum test for the miRNA binding genes vs. the rest of genes

```{r}
data3_1 <- readRDS("miRNA_motif_utr3.rds")
```
...
for (each in unique(data3$group)[grepl("mmu",unique(data3$group))]){
...
write.table(data3_1_output, "anota2seq_tab_for_genes_grouped_by_miRNA_motif_on_3UTR.tsv",sep="\t", col.names=TRUE,row.names = FALSE,quote=FALSE)

wilcox_stats <- data3_1 %>% group_by(group,wilcox_greater.p, wilcox_less.p) %>% count() %>% as.data.frame()
colnames(wilcox_stats)[4] <- "n_genes"
write.table(wilcox_stats, "translation_change_3UTR_miRNA_motif_genes_vs_allgene_wilcox_p.tsv", sep="\t", col.names=TRUE,row.names = FALSE,quote=FALSE)

saveRDS(data3_1, file="miRNA_motif_utr3.rds")
```
...
all_genes$wilcox.p <- "na"
data3_1 <- rbind(data3_1, all_genes)

p <- ggplot(data3_1, aes(x=group, y=NoGlucose_translation.apvEff)) +
  geom_boxplot(notch = TRUE, notchwidth = 0.3, width=0.6,outlier.size = 0.5) +
  theme_classic() +
  theme(legend.position="none", axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  geom_hline(yintercept = median(all_genes$NoGlucose_translation.apvEff), linetype = 2, color="red") +
  ylab("Translation log2FC") +
  xlab("Motif groups") +
  ggtitle("miRNA binding on 3'UTR") +
  ylim(-2,2) +
  # Add p-values on top
 geom_text(
    data = data3_1[,c(1,7,12)] %>% group_by(group, wilcox.p) %>% summarise(y = quantile(NoGlucose_translation.apvEff, 0.75)),
    aes(y = y,label = wilcox.p), 
    hjust = -0.1, 
    vjust = -0.1,
    size = 3, color="red",
